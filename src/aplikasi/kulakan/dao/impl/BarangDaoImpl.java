...
public class BarangDaoImpl implements BarangDao{
    ...
    public List<Barang> getAllData() {
        ...
        try{
            ...
            listBarang = session.createCriteria(Barang.class,"barang")
                    .addOrder(Order.desc("barang.idBarang")).list();
            session.flush();
    ...
    public List<Barang> findByCode(String code) {
        ...
        try{
            session = HibernateUtil.getSessionFactory().openSession();
            session.beginTransaction();
//            listBarang = (Barang)session.createCriteria(Barang.class).add(Restrictions.eq("kodeBarang", code)).uniqueResult();
            Query query = session.createQuery("from Barang a where a.kodeBarang like'%"+code+"%'");
    ...
    public void printToExcel(String path, List<Barang> listBarang) {
	    ...
	    HSSFFont fontValue = workbook.createFont();
            ...
            header[2] = "Jenis Barang";
            header[3] = "Harga/Pembagian";
            header[4] = "Deskripsi Barang";
            ...
            for(Barang barang : listBarang){
                ...
                if(barang.getTipe()==1){
                    cell.setCellValue("Titipan");
                }else{
                    cell.setCellValue("Sendiri");                    
                }
                cell.setCellStyle(cellStyle2);
                
                cell = rowData.createCell(3);
                if(barang.getTipe()==1){
                    cell.setCellValue(barang.getJumlahPembagian()+"%");
                }else{
                    cell.setCellValue(FormatRupiah.convert(String.valueOf(barang.getHarga())));                    
                }
                cell.setCellStyle(cellStyle2);
